function out = ProbabiliticFractionalIstropicTensor3D(I,sigmas,tau,tau2,D,spacing,whiteondark)
% calculates the vesselness Probabilitic Fractional anisotropy tensor of a 3D
% input image
%   -Probabilitic Fractional anisotropy tensor equation
%    Reference :
%     1- Hansen, Charles D., and Chris R. Johnson. Visualization handbook. Academic Press, 2011.â€? APA    
%     2- Prados, Ferran, et al. "Analysis of new diffusion tensor imaging anisotropy measures in the threeâ€?
%        phase plot." Journal of Magnetic Resonance Imaging 31.6 (2010): 1435-1444.â€?    

% out = ProbabiliticFractionalIstropicTensor3D(I, sigmas,spacing,tau ,tau2,D )
% 
% inputs,
%   I : 3D image
...
%
% example:
%   out = ProbabiliticFractionalIstropicTensor3D(I,[.5:.5:3],1,0.03,0.3,0.27);
%
...
for j = 1:length(sigmas)
    ...
    Trace = (abs(Lambda2)+ abs(Lambda3M) +abs(Lambda4));
    LambdaMD = 1./3;
    %%
    p2= abs(Lambda2./Trace);
    p3= abs(Lambda3M./Trace);
...
Lambda2=mu2; Lambda2(check)=mu1(check);
 ...
 %% Haifa : surface of sephere + new response
%     vt = (abs(Lambda2).*abs(Lambda2)))+abs(Lambda3+Lambda2);
%     response = response+ tanh(abs(Lambda3).*abs(Lambda2).*abs(vt));
%     response = response+ tanh((abs(Lambda3).*abs(Lambda2).^2)+ (abs(Lambda3).*abs(Lambda2).^2)+(abs(Lambda2).*abs(Lambda3).^2));
    %% Haifa :
%     response = response+ tanh(((3.*abs(Lambda3))./(abs(Lambda2)+abs(Lambda2)+abs(Lambda3))).*abs(Lambda3).*abs(Lambda2)) ;
%     response(Lambda2>=0) = 0;     
%     response(~isfinite(response)) = 0;
%     if(j==1)
%         vesselness = response;
%     else        
%         vesselness = vesselness + max(vesselness,response);
%     end